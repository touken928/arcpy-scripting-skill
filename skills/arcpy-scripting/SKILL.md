...
- 独立脚本**必须**通过 `propy.bat` 调用（见下示例）。**禁止**使用系统上的 `python`、`python3`、`py` 等通用入口直接运行依赖 ArcPy 的脚本或为其安装包；在本技能约定下，**`propy.bat` 即等效于用「ArcGIS Pro 自带的正确 Python」执行原应写成 `python …` 的命令**。
