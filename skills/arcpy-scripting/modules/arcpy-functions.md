...
本文档覆盖 ArcPy 顶层常用函数（`arcpy.func()` 形式），涵盖数据查询、描述、环境管理、许可控制、几何构造、游标（旧版）、字段处理等内容。`arcpy.management` / `arcpy.analysis` 等模块工具不在此文档范围内，请参考各模块专属文档。
...
字符串：` extension`（已授权扩展）、`Found`（基础产品）、`NotFound`（未安装）。

### 示例

```python
if arcpy.CheckProduct("ArcInfo") in ("extension", "Found"):
    print("Advanced level available")
```
...
print(installations)  # ['ArcGIS Pro']
```

### `GetInstallInfo({installation})`

返回安装信息字典。

### 参数

- `installation`（可选）：安装名称（默认 `""`）。

### 返回值

字典，键包括 `Version`、`InstallDate`、`ProductName`、`InstallPath`、`InstallType` 等。

### 示例

```python
info = arcpy.GetInstallInfo()
print(info["Version"])  # e.g., "3.6"
```
...
字符串列表，常见值：`Engine`、`Basic`、`Standard`、`Advanced`（Desktop/Pro）。

### 示例

```python
products = arcpy.ProductInfo()
print(products)  # ['Advanced']
```
